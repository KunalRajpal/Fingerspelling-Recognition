...
for file_id in tqdm(dataset_df.file_id.unique()):
    # Parquet file name
    pq_file = TRAIN_LANDMARK_PATTERN.format(file_id=file_id)
    # Filter train.csv and fetch entries only for the relevant file_id
    file_df = dataset_df.loc[dataset_df["file_id"] == file_id]
    # Fetch the parquet file
    parquet_df = pq.read_table(
        pq_file, columns=["sequence_id"] + FEATURE_COLUMNS
    ).to_pandas()
    # File name for the updated data
    tf_file = TF_FILE.format(file_id=file_id)
...
tf_records = dataset_df.file_id.map(lambda x: TF_FILE.format(file_id=x)).unique()
